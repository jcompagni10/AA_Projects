...
def bsearch(nums, target)
  ...
  if nums.length < 2
    return nums[0] == target ? 0 : nil
  end

  middle_idx = nums.length/2
  if target < nums[middle_idx]
    bsearch(nums.take(middle_idx))
  else
    ret_val = bsearch(nums.drop(middle_idx))
    retval ? middle_idx + ret_val : ret_val
  end
...
def merge_sort
  ...
  halves = [take(middle), drop(middle)]
  halves.map! {|side| side.merge_sort }
...
def merge(pt1, pt2)
  result = []
  until pt1.empty? || pt2.empty?
    least = pt1[0] < pt2[0] ? pt1.shift : pt2.shift
    result.push(least)
  end
  result + pt1 + pt2
end
