...
def charShift(char, shift)
  ascii= (char.ord-97+shift)%26+97
  ascii.chr
end
class String
  def caesar(shift)
    self.chars.map{|ch| charShift(ch,shift)}.join
  end
end
...
class Hash
  def difference(other_hash)
    all_vals = self.merge(other_hash)
    all_vals.select{|key, val| self.keys.include?(key) ^ other_hash.keys.include?(key)}
  end
end
...
class Fixnum
  def stringify(base)
    vals = ("0".."9").to_a+('a'..'f').to_a
    num = self
    str = ''
    idx = Math.log(num,base).floor
    while idx >= 0
      power = base ** idx
      idx -= 1
      div = num / power
