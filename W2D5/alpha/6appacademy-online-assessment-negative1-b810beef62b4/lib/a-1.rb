...
def missing_numbers(nums)
  ...
  least = sorted_nums[0]
  greatest = sorted_nums[-1]
  result = []
  (least...greatest).select { |num| !nums.include?(num)}
end
...
def base2to10(binary)
  digits = get_digits(binary.to_i)
  result = 0
  digits.each_with_index {|digit| result += digit * 2 ^ index}
end
