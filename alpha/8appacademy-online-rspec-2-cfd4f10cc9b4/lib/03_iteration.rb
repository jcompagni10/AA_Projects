...
class Array
  def bubble_sort!(&prc)
    ...
    while changes_made
      changes_made = false
      self[0..-2].each_with_index do |val, idx|
        if prc.call(val, self[idx+1]) == 1
...
def substrings(string)
  ...
  (0..string.length-1).each do |start|
    (start..string.length).each{ |stop| substrings << string[start..stop] }
  end
  substrings.uniq
end
...
class Array
  ...
  def my_inject(&blk)
  agg = self[0]
  self[1..-1].my_each{|val| agg = blk.call(agg, val)}
  agg
  end
end
