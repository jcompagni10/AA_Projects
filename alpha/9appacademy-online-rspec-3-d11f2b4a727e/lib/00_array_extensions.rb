...
class Array
  def two_sum
    twos = []
    self[0..-2].each_with_index do |a, idx1|
      self[idx1+1..-1].each_with_index do |b, idx2|
        twos << [idx1, idx2+idx1+1] if a +b == 0
      end
    end
    twos
  end
...
class Array
  def my_transpose
    ...
    self.each_with_index do |row, row_idx|
      row.each_with_index{|val, col_idx| new_arr[col_idx][row_idx] = val}
    end
    new_arr
  end
end
