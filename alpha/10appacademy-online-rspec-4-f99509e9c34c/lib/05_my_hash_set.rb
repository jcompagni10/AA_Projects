...
class MyHashSet
  ...
  def union(set)
    ...
    vals = set.to_a + @store.keys
    vals.each{|val| newSet.insert(val)}
    newSet
  end
  ...
  def intersect(set)
    ...
    vals = set.to_a
    vals.each do |val|
      newSet.insert(val) if self.include?(val)
    end
    newSet
  end
  ...
  def minus(set)
    vals = set.to_a
    new_set = dup
    vals.each{|val| new_set.delete(val)}
    new_set
  end

  def symmetric_difference(set)
    overlap =  intersect(set)
    union(set).minus(overlap)
  end

  def ==(object)
    return false if object.class != MyHashSet
    test_set = dup
    same_values = object.to_a.all?{|val| test_set.delete(val)}
    same_values && test_set.to_a.length == 0
  end
